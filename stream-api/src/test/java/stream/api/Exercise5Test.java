...
public class Exercise5Test extends ClassicOnlineStore {
    ...
    public void ageDistribution() {
         ...
         */
        Map<Integer, List<Customer>> groupped = customerList.stream()
                .collect(Collectors.groupingBy(customer -> customer.getAge()));

        Map<Integer, Optional<Customer>> budgetDistribution = customerList.stream()
                .collect(Collectors.groupingBy(
                        customer -> customer.getAge(),
                        Collectors.maxBy(Comparator.comparing(Customer::getBudget))
                ));
