...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream
                .flatMap(c -> c.getWantToBuy().stream().map(Item::getName))
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = c -> c.getWantToBuy().stream()
                .mapToInt(i -> findCheapestItem(onSale, i)).sum() < c.getBudget();
    ...
    private Integer findCheapestItem(List<Item> onSale, Item i) {
        return onSale.stream()
                .filter(onSaleItem -> onSaleItem.getName().equals(i.getName()))
                .findFirst()
